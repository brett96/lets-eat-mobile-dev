...
### Fourth pass — Sign in with Apple + launcher icons (also on this branch)

- **Sign in with Apple** — implemented via `sign_in_with_apple` with the secure nonce flow (`AuthService.signInWithApple`, using `crypto` for the SHA-256 nonce). Firebase links it by identity token; Apple's first-authorization-only name is captured into the display name and the `usernames` directory. A "Continue with Apple" button appears on the login screen only on Apple platforms. Satisfies the App Store requirement to offer Apple sign-in wherever Google Sign-In is present. The two remaining steps are account/portal-only (Xcode capability + enabling the Apple provider in Firebase) — documented in `ios/FIREBASE_SETUP.md`.
- **Launcher icons** — `flutter_launcher_icons` configured in `pubspec.yaml` from `assets/ACTUALAPPLOGO.png` (512²) and generated for both platforms: Android adaptive icons (`mipmap-anydpi-v26` + `colors.xml`, white background) plus legacy densities, and the full iOS `AppIcon.appiconset` with alpha stripped (`remove_alpha_ios`) for App Store compliance. Regenerate with `dart run flutter_launcher_icons`.

**Remaining:** register the iOS app in the Firebase console + add `GoogleService-Info.plist`, APNs key, and the Sign in with Apple / Apple provider setup (all in `ios/FIREBASE_SETUP.md`); remaining store-readiness items in REFACTOR_PLAN.md Phase 5 (privacy policy, data-safety/privacy-manifest forms, store listings, screenshots). And before release: rotate the exposed Yelp + Maps keys (legacy copies remain in git history), and audit `firestore.rules` against real data using the emulator.
